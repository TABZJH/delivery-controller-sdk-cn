...
重新加载 ADIdentity Service 的访问权限和配置服务位置。
...
用于重新加载 ADIdentity Service 访问权限和配置服务位置。 在安装并向配置服务注册后，必须至少在 (Acct) 服务类型的一个实例上运行 Reset-AcctServiceGroupMembership 命令。 如果不执行此操作，ADIdentity Service 将不能与 XenDesktop 部署中的其他服务进行通信。 运行该命令时，如果配置服务未停止，向部署添加其他服务时，服务会更新。 如果向部署添加新服务时未自动更新，可以重新运行 Reset-AcctServiceGroupMembership 命令以刷新此信息。 如果向命令传递多个配置服务实例，则使用符合预期服务类型要求的第一个实例。
...
包含 ServiceInstance 对象（引用了集中式配置服务 InterService 接口）的服务实例可以通过管道传送到 Reset-AcctServiceGroupMembership 命令。
